    ...
    class Army : Script
    {
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                case 34:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't serve in the army", 3000);
                        return;
                    }
                ...
                case 36:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Maximum amount of material in the car", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Downloads started (20 seconds)", 3000);
                ...
                case 60: // open stock gun
                    ...
                    GUI.Dashboard.OpenOut(player, Stocks.fracStocks[14].Weapons, "Weapons stock", 6);
                    return;
            }
        }
            ...
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    if (player.GetData<string>("whereLoad") == "WAR" && !Fractions.MatsWar.isWar)
                    {
                        ...
                        player.ResetData("loadMatsTimer");
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The ship has already left", 3000);
                        return;
                    }
        ...
        public static void Event_PlayerDeath(Player player, Player entityKiller, uint weapon)
        {
            try
            {
                ...
                {
                    Trigger.ClientEvent(player, "hideLoader");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Uploads canceled because you died", 3000);
        ...
        public void onVehicleDeath(Vehicle vehicle)
        {
            try
            {
                if (NAPI.Data.HasEntityData(vehicle, "loaderMats"))
                {
                    ...
                    player.ResetData("loadMatsTimer");
                    Trigger.ClientEvent(player, "hideLoader");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Loading of materials canceled because the car left the checkpoint", 3000);
                }
            }
        ...
        private static void callback_armyclothes(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "change":
                    ...
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You can not change clothes", 3000);
                        return;
                    }
                    if (!client.GetData<bool>("ON_DUTY"))
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed into a service uniform", 3000);
                        Manager.setSkin(client, 14, Main.Players[client].FractionLVL);
                        client.SetData("ON_DUTY", true);
                    }
                    else
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed into casual clothes", 3000);
        ...
        private static void callback_armycombat(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't serve in the army", 3000);
                return;
            }
            if (!client.GetData<bool>("ON_DUTY"))
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You must start the service", 3000);
                return;
            }
