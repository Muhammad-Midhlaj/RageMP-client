    ...
    class Police : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Press E to open the menu"), new Vector3(policeCheckpoints[1].X, policeCheckpoints[1].Y, policeCheckpoints[1].Z + 0.7), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Turn in your bags and get a reward"), new Vector3(policeCheckpoints[7].X, policeCheckpoints[7].Y, policeCheckpoints[7].Z + 0.7), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Open stock"), new Vector3(policeCheckpoints[8].X, policeCheckpoints[8].Y, policeCheckpoints[8].Z + 0.7), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Improvement"), new Vector3(policeCheckpoints[9].X, policeCheckpoints[9].Y, policeCheckpoints[9].Z + 0.7), 5F, 0.3F, 0, new Color(255, 255, 255));
        ...
        public static void callPolice(Player player, string reason)
        {
            NAPI.Task.Run(() =>
            {
                try {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "There are no police officers in your area. try later", 3000);
                        return;
                    }
                    if (player.HasData("NEXTCALL_POLICE") && DateTime.Now < player.GetData<DateTime>("NEXTCALL_POLICE"))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already called the police, please try again later", 3000);
                        return;
                    }
                    ...
                    colshape.OnEntityExitColShape += (s, e) =>
                    {
                        if (e == player)
                        {
                            try
                            {
                                ...
                                Manager.sendFractionMessage(7, $"{e.Name.Replace('_', ' ')} canceled the call");
                                Manager.sendFractionMessage(9, $"{e.Name.Replace('_', ' ')} canceled the call");
        ...
        public static void clearWantedLvl(Player sender, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight, $"There is no passport with this number", 3000);
                        return;
                    }
                ...
                catch
                {
                    ...
                    {
                        Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomRight, $"Player not found", 3000);
                        return;
                    }
                ...
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomRight, $"You have removed the wanted list from the passport holder {target}", 3000);
        ...
        public static void checkNumber(Player sender, params object[] arguments)
        {
            try
            {
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"No car with this number found", 3000);
                    return;
                }
            ...
            try
            {
                    ...
                    (sender.VehicleSeat == 0 || sender.VehicleSeat == 1))
                {
                    ...
                    if (Main.Players[sender].FractionID == 7 || Main.Players[sender].FractionID == 9)
                    {
                        ...
                        Commands.RPChat("me", sender, "turned on the on-board computer");
                    }
                }
                return;
            }
        ...
        public static void interactPressed(Player player, int interact)
        {
            ...
            switch (interact)
            {
                case 10:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a police officer", 3000);
                        return;
                    }
                ...
                case 11:
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a police officer", 3000);
                    return;
                case 12:
                    if (Main.Players[player].FractionID != 7)
                    {
                    ...
                    if (!is_warg)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Emergency mode not enabled", 3000);
                        return;
                    }
                ...
                case 42:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have a bag of money or a bag with a drill.", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You received a reward of $ 200", 3000);
                    return;
                case 44:
                    if (Main.Players[player].Licenses[6])
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You already have a gun license", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a gun license", 3000);
                ...
                case 66:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in a working machine", 3000);
                        return;
                    }
                    Trigger.ClientEvent(player, "svem", 20, 20);
                    player.Vehicle.SetSharedData("BOOST", 20);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Boosted", 3000);
                    return;
            }
        }
        ...
        public static void onPlayerDisconnectedhandler(Player player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                if (player.HasData("CALLPOLICE_BLIP"))
                {
                    ...
                    Manager.sendFractionMessage(7, $"{player.Name.Replace('_', ' ')} canceled the call");
                    Manager.sendFractionMessage(9, $"{player.Name.Replace('_', ' ')} canceled the call");
                }
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 5:
                        ...
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough material in stock", 3000);
                            return;
                        }
                        var aItem = nInventory.Find(Main.Players[client].UUID, ItemType.BodyArmor);
                        if (aItem != null)
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "You already have a bulletproof vest", 3000);
                            return;
                        }
                        ...
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You received a bulletproof vest", 3000);
                    ...
                    case 6: // medkit
                        ...
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "No first aid kits in stock", 3000);
                            return;
                        }
