...
SELECT * FROM questions;";
        ...
        public static void AddReport(Player player, string question)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You sent вопрос: {question}", 3000);
        ...
        private static void AddAnswer(Player player, int repID, string response)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (target is null)
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Player not found!", 3000);
                    }
                    else
                    {
