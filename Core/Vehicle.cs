                    ...
                    if (NAPI.Data.GetEntityData(vehicle, "ACCESS") == "FRACTION")
                    {
                        if (NAPI.Data.GetEntityData(vehicle, "FRACTION") == 14 && vehicle.DisplayName == "Barracks")
                        {
                            ...
                            if ((fracid >= 1 && fracid <= 5) || (fracid >= 10 && fracid <= 13))
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"It is impossible to get in the car from 00:00 to 10:00", 3000);
                                    return;
                                }
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Press B to start the engine", 3000);
                                return;
                            }
                            else if (fracid == 14)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have access to this transport", 3000);
        ...
        public static void ChangeVehicleDoors(Player player, Vehicle vehicle)
        {
            switch (NAPI.Data.GetEntityData(vehicle, "ACCESS"))
            {
                case "HOTEL":
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have the keys to this vehicle", 3000);
                        return;
                    }
                ...
                case "engineCarPressed":
                    ...
                    switch (NAPI.Data.GetEntityData(vehicle, "ACCESS"))
                    {
                        case "HOTEL":
                            ...
                            {
                                Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have the keys to this vehicle", 3000);
                                return;
                            }
                            ...
                            else
                            {
                                ...
                                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"You engine the car", 3000);
                            }
                            break;
                        case "SCHOOL":
                        ...
                        case "beltCarPressed":
                            ...
                            if (!beltstate) Commands.RPChat("me", sender, "fastened the seat belt");
                            else Commands.RPChat("me", sender, "unfastened the seat belt");
                    ...
                    if (Core.VehicleStreaming.GetEngineState(vehicle)) Commands.RPChat("me", sender, "engine the vehicle");
                    else Commands.RPChat("me", sender, "drowned out the vehicle");
                    return;
            ...
            try
            {
                ...
                switch (access)
                {
                    case "PERSONAL":
                        {
                            ...
                            Notify.Send(owner, NotifyType.Alert, NotifyPosition.BottomCenter, "Your car is destroyed", 3000);
                    ...
                    case "WORK":
                        ...
                        if (player != null)
                        {
                            string paymentMsg = (player.GetData<int>("PAYMENT") == 0) ? "" : $"You received a salary in {player.GetData<int>("PAYMENT")}$";
